        ...
        <div className="p-8 space-y-8 bg-slate-50/50 dark:bg-transparent min-h-screen">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Comprehensive overview of your store performance.</p>
                </div>

                <form onSubmit={handleFilter} className="flex flex-wrap items-end gap-3 bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border dark:border-slate-800">
                    <div className="space-y-1.5">
                        <Label htmlFor="startDate" className="text-xs uppercase font-bold text-slate-400">Start Date</Label>
                        <Input
                            type="date"
                            id="startDate"
                            value={startDate}
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="endDate" className="text-xs uppercase font-bold text-slate-400">End Date</Label>
                        <Input
                            type="date"
                            id="endDate"
                            value={endDate}
                    ...
                    <Button type="submit" size="sm" variant="secondary" disabled={loading} className="gap-2 h-9">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((card) => (
                    <Card key={card.title} className="border-none shadow-sm hover:shadow-md transition-all duration-200 dark:bg-slate-900 group">
                        <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                            <CardTitle className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{card.title}</CardTitle>
                            <div className={`${card.bg} p-2 rounded-lg transition-transform group-hover:scale-110`}>
                ...
                <Card className="dark:bg-slate-900 border-none shadow-sm">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-red-600">
                            <AlertCircle className="h-5 w-5" /> Critical Alerts
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex items-center p-4 bg-red-50 dark:bg-red-900/10 text-red-700 dark:text-red-400 rounded-xl text-sm border border-red-100 dark:border-red-900/20">
                            <AlertCircle className="mr-3 h-5 w-5 shrink-0" />
                            System Audit: All stock levels currently satisfy minimum requirements.
                        </div>
                    </CardContent>
                </Card>
            </div>
