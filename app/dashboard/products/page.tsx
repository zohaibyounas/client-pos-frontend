...
export default function ProductPage() {
    ...
    return (
        <div className="min-h-screen space-y-8 bg-slate-50 p-6 dark:bg-slate-950 md:p-8">
            ...
            {/* Product cards — same style as Dashboard blocks (no line, compact) */}
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {products.length === 0 ? (
                    <Card className="border-slate-200 py-12 dark:border-slate-800 dark:bg-slate-900 md:col-span-2 lg:col-span-4">
                        <CardContent className="flex flex-col items-center justify-center text-center">
                            <Package className="mx-auto mb-3 h-10 w-10 text-slate-300 dark:text-slate-600" />
                            <p className="text-slate-500 dark:text-slate-400">No products yet. Add your first product to get started.</p>
                            <Button className="mt-4 bg-blue-600 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700" onClick={() => setShowAdd(true)}>
                    ...
                    products.map((p) => (
                        <Card
                            key={p._id}
                            className="overflow-hidden border-slate-200 shadow-sm transition-all duration-200 hover:border-blue-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-700"
                        >
                            {p.image ? (
                                <img src={`http://localhost:5000${p.image}`} alt={p.name} className="h-36 w-full object-cover" />
                            ) : (
                            ...
                            <CardHeader className="p-3 pb-1">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0 flex-1">
                                        <CardTitle className="truncate text-base font-bold text-slate-900 dark:text-white">{p.name}</CardTitle>
                                        <p className="truncate text-xs text-slate-500 dark:text-slate-400">{p.category || '—'}</p>
                                    </div>
                                    <div className="flex shrink-0 items-center gap-1">
                                        <span className="rounded-md bg-blue-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                                            {p.totalStock}
                                        </span>
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/30" onClick={() => handleEdit(p)}>
                                            <Edit className="h-4 w-4" />
                                        </Button>
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20" onClick={() => handleDelete(p._id)}>
                            ...
                            <CardContent className="p-3 pt-1">
                                <div className="flex items-baseline justify-between gap-2">
                                    <span className="text-lg font-bold text-emerald-600 dark:text-emerald-400">Rs. {Number(p.salePrice).toLocaleString()}</span>
                                    <span className="text-[10px] text-slate-400 line-through dark:text-slate-500">Rs. {Number(p.costPrice).toLocaleString()}</span>
                                </div>
                                <p className="mt-1.5 truncate text-[10px] font-mono text-slate-500 dark:text-slate-400">Barcode: {p.barcode}</p>
                            </CardContent>
                        </Card>
                    ))
                )}
